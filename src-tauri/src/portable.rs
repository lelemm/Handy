...
/// Portable mode support for (not)Handy.
///
...
const PORTABLE_MARKER: &str = "(not)Handy Portable Mode";
...
mod tests {
    ...
    fn test_valid_magic_string_enables_portable() {
        let dir = std::env::temp_dir().join("not_handy_test_valid");
    ...
    fn test_legacy_magic_string_enables_portable() {
        let dir = std::env::temp_dir().join("not_handy_test_legacy_valid");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        let mut f = std::fs::File::create(&marker).unwrap();
        write!(f, "{LEGACY_PORTABLE_MARKER}").unwrap();
        assert!(is_valid_portable_marker(&marker));
        std::fs::remove_dir_all(dir).unwrap();
    }
    ...
    fn test_empty_file_does_not_enable_portable() {
        let dir = std::env::temp_dir().join("not_handy_test_empty");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
    ...
    fn test_wrong_content_does_not_enable_portable() {
        let dir = std::env::temp_dir().join("not_handy_test_wrong");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        let mut f = std::fs::File::create(&marker).unwrap();
    ...
    fn test_legacy_empty_marker_without_data_dir_does_not_enable_portable() {
        ...
        let dir = std::env::temp_dir().join("not_handy_test_legacy_no_data");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        std::fs::File::create(&marker).unwrap();
        assert!(!is_valid_portable_marker(&marker));
    ...
    fn test_magic_string_with_whitespace_enables_portable() {
        let dir = std::env::temp_dir().join("not_handy_test_ws");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        let mut f = std::fs::File::create(&marker).unwrap();
        write!(f, "  {PORTABLE_MARKER}\n").unwrap();
        assert!(is_valid_portable_marker(&marker));
        std::fs::remove_dir_all(dir).unwrap();
    }
}
